(function (){
  'use strict';
  const userNameInput = document.getElement('user-name');
  const assessmentButton = document.getElementById('assessment');
  const resultDivided = document.getElement('result-area');
  const tweetDivided = document.getElement('tweet-area');
  ...
  */
  ...
  const answers = [
    ...
    '{userName}のいいところは用心深さです。{userName}の洞察に多くの人が助けられてます。',
  ...
  */
  function assessment(userName) {
    ...
    for (let i = 0; i < userName.length; i++) {
      sumOfcharCode = sumOfcharCode + userName.
      charCodeAt(i);
    }
